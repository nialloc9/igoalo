<?php $profile_pic = User::getUserProfileImage(htmlspecialchars($user->profile_image_upload_location)); ?>
...
<div id="hide_<?php echo htmlspecialchars($status->id); ?>" class="media replyblock_container">
    <div class="replyblock_image_wrapper" id="reply_profile<?php echo htmlspecialchars($status->id); ?>">
        <a class="pull-left" href="<?php echo 'profile_template_user.php?page_id='.htmlspecialchars($user->id); ?>); ?>">
            <img class="media-object img-rounded replyblock_image" alt="<?php echo htmlspecialchars($user->username); ?>" src="<?php echo $profile_pic; ?>">
        </a>
    </div>
    ...
    <div class="media-body">
        <h4 class="media-heading"><a href="<?php echo 'profile_template_user.php?page_id='.htmlspecialchars($user->id); ?>); ?>"><?php echo htmlspecialchars($user->username); ?></a></h4>
        <p id="status_body"><?php echo $status->body; ?></p>
        <ul class="list-inline">
            ...
            <?php if(!User::authIsUser($_SESSION['user_id'], htmlspecialchars($user->id))): ?>
            ...
            <?php if(User::authIsUser($user_id, $status->user_id)): ?>
                ...
                <input type="hidden" id="status_delete_token" value="<?php if(isset($token)){ echo $token;}else{echo $csrf_token;} ?>"/>
            <?php endif ?>
            <?php ?>
