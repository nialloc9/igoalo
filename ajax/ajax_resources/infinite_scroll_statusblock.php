...
<div id="status_edit_show_<?php echo htmlspecialchars($status_edit->id); ?>" class="media col-sm-9 status-container hidden">
    ...
    <div class="media-body edit_status_text_box">
        <form action="" method="post">
            ...
            <p id="status_body"><textarea name="status_body_edit" class="form-control" rows="5"><?php echo strip_tags($status_edit->body, '<br>'); ?></textarea></p>
            <?php if(isset($status_edit->image) && !empty($status_edit->image)): ?>
                <p id="status_body"><a><img class="status_post_image" src="<?php echo htmlspecialchars($status_edit->image); ?>"</a></p>
            <?php elseif(isset($status_edit->video) && !empty($status_edit->video)): ?>
                <p id="status_body">
                    <video controls>
                        <source src="<?php echo htmlspecialchars($status_edit->video); ?>" type="video/mp4">
                        Your browser does not support the video tag.
                    </video>
                </p>
            <?php elseif(isset($status_edit->attachment) && !empty($status_edit->attachment)): ?>
                <p id="status_body"><!-- Attachment goes here --></p>
            <?php elseif(isset($status_edit->goal_id) && !empty($status_edit->goal_id)): ?>
                <?php $s_goal = Goals::get_goal(htmlspecialchars($status_edit->goal_id), $db); $status_goal = $s_goal[0][0]; ?>
                <?php if(isset($status_goal) && !empty($status_goal)): ?>
                    <?php require 'ajax_timeline_status_goal.php'; ?>
                <?php endif; ?>
            <?php elseif(isset($status_edit->group_id) && !empty($status_edit->group_id)): ?>
