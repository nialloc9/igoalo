$(document).ready(function(){
    ...
    $('#group_status_post').click(function(){
        group_status_post_click();
    });
});

function group_status_post_click(){
    ...
    if(_status.length >0 || _status_image > 0 || _status_video > 0 || _attachment > 0 && _user_id != null && _token == '1'){

        $.post('ajax/status_insert.php',
            {
                task: 'group_status_insert',
                user_id: _user_id,
                status: _status,
                video: _status_video,
                image: _status_image,
            ...
            function(data) {
                ...
                $('#status_body').val('');
                group_status_insert($.parseJSON(data));
            }
        );

        //console.log('  Status  ' + _status + '  user_id:  ' + _user_id + '  Video:  ' + _status_video + ' image: ' + _status_image + ' attachment ' + _attachment + ' parent id ' + _parent_id + ' group_id ' + _group_id + ' posted at '+ _posted_at);
        //Proceed
    }else{
...
function group_status_insert(data){
    var t = '';
    var type = 1;
    ...
    t+= '<textarea name="reply-" class="form-control" rows="2" placeholder="Reply to this status"></textarea>';
    t+= '<span class="help-block"></span>';
    t+= '</div>';
    t+= '<input type="submit" value="Reply" class="btn btn-default btn-sm">';
    t+= '<input type="hidden" value="token">';
    t+= '</form>';
    t+= '</div>';
    t+= '</div>';
