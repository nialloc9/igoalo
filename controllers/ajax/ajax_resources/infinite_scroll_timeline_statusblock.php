...
<div id="status_edit_show_<?php echo htmlspecialchars($status_edit['id']); ?>" class="media col-sm-9 status-container hidden">
    <div class="status-image-wrapper">
        <a class="user_redirect" onclick="page_redirect(<?php echo htmlspecialchars($user->id); ?>)" id="<?php echo htmlspecialchars($user->id); ?>" href="profile_template_user.php">
    ...
    <div class="media-body edit_status_text_box">
        <form action="" method="post">
            <h4 class="media-heading"><a id="<?php echo htmlspecialchars($user->id); ?>" href="<?php echo 'profile_template_user.php?page_id='.htmlspecialchars($user->id); ?>); ?>"><?php echo htmlspecialchars($user->username); ?></a></h4>
...
<div id="hide_<?php echo $status['id']; ?>" class="media col-sm-9 status-container">
    <div class="status-image-wrapper">
        <a class="pull-left" id="<?php echo htmlspecialchars($user->id); ?>" href="<?php echo 'profile_template_user.php?page_id='.htmlspecialchars($user->id); ?>); ?>">
            <img class="media-object img-rounded status-image" alt="<?php echo htmlspecialchars($user->username); ?>" src="<?php echo $user_profile_status_image; ?>">
        </a>
    </div>
    ...
    <div class="media-body status_body">
        <h4 class="media-heading"><a id="<?php echo htmlspecialchars($user->id); ?>" href="<?php echo 'profile_template_user.php?page_id='.htmlspecialchars($user->id); ?>); ?>"><?php echo htmlspecialchars($user->username); ?></a></h4>
        <p class="status_posted_in"><?php if(isset($group) && !empty($group)): ?>posted in : <a  class="groupblock-image-holder" id="<?php echo htmlspecialchars($group['id']); ?>" href="<?php echo 'profile_template_group.php?group_id='.htmlspecialchars($group['id']); ?>); ?>"><?php echo htmlspecialchars($group['group_name']); ?></a><br><br><?php endif; ?></p>
        ...
        <ul class="list-inline">
            ...
            <?php elseif(User::authIsUser($user_id, htmlspecialchars($status['user_id']))): ?>
                <li class="status_delete btn" id="<?php echo htmlspecialchars($status['id']); ?>"><a>Delete</a></li>
            <?php endif ?>
